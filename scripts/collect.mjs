...
boards.newProjects = allRepositories.filter((repo) => repo.createdAt && Date.now() - new Date(repo.createdAt).getTime() <= 30 * 86400000 && repo.starGrowth24h !== null).sort((a, b) => b.starGrowth24h - a.starGrowth24h || b.stars - a.stars).slice(0, TOP).map((repo, index) => ({ ...repo, rank: index + 1 }));
